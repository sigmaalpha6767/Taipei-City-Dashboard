...
const TOOLS = [
	...
	},
	{
		...
		function: {
			...
			description: "取得多次違規累犯店家清單。回傳店家名稱、違規次數、城市、所在行政區、主要違規類別、相關食材。當使用者問「累犯」「該稽查的店」「它們在哪裡」之類接續問題時呼叫。",
			parameters: {
				type: "object",
...
- 使用者問食安事件、累犯店家、行政區風險、學校長照影響 → 呼叫對應食安工具
- 使用者問「它們」「他們」「那些」這類代名詞 → 從上下文找出指涉對象，必要時重新呼叫工具取得補充資訊（如店家所在地、累犯細節等）
